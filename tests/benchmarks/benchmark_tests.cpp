...
bool testOptions()
{
    ...
    if (!parse({"benchmark"}, options)
        ...
        || options.solverWorkers != 1 || options.broadPhaseWorkers != 1)
        return false;
    if (!parse({"benchmark", "--sleep"}, options) || !options.sleepingEnabled
        || options.measuredSteps != 1200 || options.warmupSteps != 240)
        return false;
    if (!parse({"benchmark", "9", "tree", "mixed", "4", "3", "2", "5", "--sleep"}, options)
        ...
        || options.narrowPhaseWorkers != 3 || options.solverWorkers != 2
        || options.broadPhaseWorkers != 5)
        return false;
    ...
    return !parse({"benchmark", "0"}, options)
        ...
        && !parse({"benchmark", "5", "tree", "mixed", "0", "1", "0"}, options)
        && !parse({"benchmark", "5", "tree", "mixed", "0", "1", "1", "0"}, options)
        && !parse({"benchmark", "--sleep", "--sleep"}, options)
        && !parse({"benchmark", "5", "tree", "mixed", "0", "1", "1", "1", "extra"}, options);
}
...
bool testWarmupAndReport()
{
    ...
    if (report.sampleCount != 120 || report.warmupSteps != 240
        ...
        || report.narrowPhaseWorkers != 1 || report.solverWorkers != 1
        || report.broadPhaseWorkers != 1
    ...
    if (output.str().find("Dynamic bodies:             32") == std::string::npos
        ...
        || output.str().find("Solver workers:             1") == std::string::npos
        || output.str().find("Broadphase workers:         1") == std::string::npos
